...
def ctrDecrypt1(key, cypherText, blockSize):
    k = binascii.a2b_hex(bytes(key, encoding='utf-8'))
    ct = binascii.a2b_hex(bytes(cypherText, encoding='utf-8'))
    # k = key.decode('hex')
...
def ctrDecrypt2(key, cypherText, blockSize):
    ...
    cypherTextBlocksDecoded = map(
        methodcaller("decode", "hex"), cypherTextBlocks)
...
def cbcDecrypt2(key, cypherText, blockSize):
    ...
    iter = len(list(cypherTextBlocksDecoded))
